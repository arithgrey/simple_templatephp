...
function update_q(campo ,  valor ,  place_load ){
	...
	$.ajax({
		...
		beforeSend:  function(){
			llenaelementoHTML(place_load ,  "Registrando cambios ... " );
		}
	}).done(function(data){		

		msj =  "<span class='data_actualizada'>Datos actualizados con éxito </span>";
		llenaelementoHTML(place_load , msj );

	}).fail(function(){
		llenaelementoHTML( place_load  ,  "Error al registrar los cambios, reporte al administrador");        
	});
}
/**/
...
function t_e_descripcion_user(){
	...
	flag =  valida_text_form("#descripcion_user" , ".place_descripcion" , 255 , "El texto que redactaste para tu descripción  " );	
...
function  actualiza_password(anterior , nuevo , confirma){
	...
	$.ajax({
			...
			beforeSend : function(){
				llenaelementoHTML(".msj_password" , "Registrando nueva contraseña ");
			}
		}).done(function( data ){					

			if (data == true){	
				llenaelementoHTML(".msj_password" , "Contraseña actualizada correctamente, inicia sessión para verificar el cambio");					
		...
		llenaelementoHTML(".msj_password" , genericresponse[0] );		
	});	
}
