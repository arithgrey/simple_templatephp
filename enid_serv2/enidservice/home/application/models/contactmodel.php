<?php defined('BASEPATH') OR exit('No direct script access allowed');
 class contactmodel extends CI_Model {  
  ...
  function get_contactos_user($idusuario , $param  , $limit=10 ){
    ...
    $filtro ="";
    if ( strlen($param["contacto-name"]) > 0 and  strlen($param["contacto-tel-filtro"]) > 0 ) {
    ...
    }elseif (strlen($param["contacto-name"]) > 0 ) {
        $filtro= " and (nombre  like '". $param["contacto-name"] ."%'  and   tipo like '". $param["filtro-tipo-contacto"] ."%')" ;        
    }
    ...
    }else if(strlen($param["contacto-name"]) < 1 and  strlen($param["contacto-tel-filtro"]) < 1){
    ...
    $query_get ="SELECT c.* , i.* , c.status  estado_contacto  , c.fecha_registro   fecha_registro_contacto   FROM contacto c  left outer  join  imagen_contacto ic on 
