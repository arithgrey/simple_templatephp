...
/**/
function update_descripcion_escenario_template($param){	
...
function nuevo( $nombre , $evento ,  $id_empresa  , $id_usuario , $nombre_usuario ){
	...
	if ($id_escenario > 0 ){		
		/**/
		$actividad =  $nombre_usuario ." cargó el escenario " . $nombre . " al evento ";
		$query_insert = "INSERT INTO log_evento (actividad , id_usuario , idempresa ,  id_evento , tipo , accion ) VALUES( '". $actividad ."' ,  '".$id_usuario."' , '". $id_empresa ."' , '".$evento ."' , 2 , 'insert') ";		
		$this->db->query($query_insert);
	}
...
function get_escenarios_byidevent($id_evento){
	...
	$query_get ="select e.* , e.idescenario id_escenario,  e.descripcion  as descripcion_escenario , 
...
function load_by_event( $id_evento ,  $idempresa  ){
	...
	$query_get ="SELECT e.idescenario , e.nombre , e.descripcion ,   e.idevento , e.tipoescenario
...
function updateescenariotipobyid($idescenario , $tipoescenario , $idempresa){
	...
	$query_upload ="UPDATE  escenario set tipoescenario = '$tipoescenario' WHERE   idescenario ='$idescenario' limit 1";
	$result = $this->db->query($query_upload);
	return $result;	
...
function get_escenariobyId($id_escenario){
	...
	$query_get ="SELECT * FROM escenario WHERE idescenario ='".$id_escenario."' ";
	$result = $this->db->query($query_get);
	return $result->result_array();	
}
