...
/**/
function registraartistaescenario($idescenario , $nuevoartista , $idempresa, $id_evento , $id_usuario){

	$registroartista = $this->nuevoartista($nuevoartista);
	if ($registroartista[0] ==  true) {
		$idartista = $registroartista[1];
		return $this->nuevo_escenario_artista($idescenario , $idartista , $id_evento , $nuevoartista , $id_usuario , $idempresa );
	}else{
...
/**/
function nuevo_escenario_artista($idescenario , $idartista , $id_evento , $artista , $id_usuario  , $idempresa ){
		...
		$query_insert = "INSERT INTO evento_artista(id_evento , id_artista, artista )VALUES( '". $id_evento ."' , '".$idartista  ."' , '". $artista ."')";
		$result = $this->db->query($query_insert);

		/*Cargamos ahora el log*/
		$actividad =  "Indicó que " .  $artista ." se presentará en el evento "; 
		$query_insert = "INSERT INTO log_evento(actividad , id_usuario , idempresa , id_evento , tipo , accion  ) 
						VALUES('".$actividad."' , '". $id_usuario."' , '". $idempresa."' , '". $id_evento."' , '4' , 'insert' )"; 
...
/******************************************************************/
function deleteescenarioartosta($idescenario , $artista_quitar , $idempresa , $id_evento ){
	...
	$query_delete = "DELETE FROM evento_artista WHERE id_evento = '". $id_evento  ."' AND id_artista = '". $artista_quitar ."'  ";
