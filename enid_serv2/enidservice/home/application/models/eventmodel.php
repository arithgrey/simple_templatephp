...
function get_resum_by_id_event($id_evento){
	...
	$query_procedure = "call enidserv_eniddbbbb3.data_event_public($id_evento);";	
...
function update_tematicaby_id( $idevento , $idtematica, $idempresa ){
	...
	$query_delete ="DELETE FROM evento_tematica WHERE idevento ='".$idevento."'";
...
/**/
function create( $nombre , $edicion , $inicio , $termino , $idusuario , $idempresa , $perfiles , $nombre_usuario ) {
	...
	/*Registramos log evento */
		if ($dbresponse) {		
			/*Registramos log */
			$id_ultimo_evento = $this->db->insert_id(); 							
			$actividad =  $nombre_usuario . " registró el evento " . $nombre ." con fecha de inicio " .  $inicio; 

			$query_insert =  "INSERT INTO  log_evento(actividad  , id_usuario , idempresa , id_evento  , tipo , accion ) 
								VALUES('".$actividad."' , '". $idusuario  ."' ,  '". $idempresa ."' , '". $id_ultimo_evento ."' , 1 , 'insert' ) ";  
			$this->db->query($query_insert);						

			/**/
			$this->update_diferencias_dias($id_ultimo_evento); 
			return $id_ultimo_evento;
...
function get_last_events($idempresa , $num ){
	...
	$query_temporal_tables =  "call enidserv_eniddbbbb3.repo_eventos_admin($idempresa);"; 
	$this->db->query($query_temporal_tables);	

$query_get ="select e.*  , ea.artistas , epv.evento_punto_venta, es.servicios , ra.accesos 
, CONCAT(i.base_path_img , i.nombre_imagen)portada      from repo_eventos_escenarios_$idempresa e
left outer join  repo_escenarios_artistas_$idempresa ea 
on e.idevento = ea.idevento 
left outer join repo_evento_puntos_venta_$idempresa epv 
on  e.idevento =  epv.idevento
left outer join repo_evento_servicios_$idempresa  es 
on e.idevento =  es.idevento
left outer join reporte_evento_accesos_$idempresa ra 
...
function getEventbyid($idevento){
	...
	$query_get ="SELECT * FROM evento where idevento ='". $idevento ."' ";
	$result = $this->db->query($query_get);
	return $result ->result_array();      
}
...
function updateNombre($nuevo_nombre , $idevento ){
	...
	$query_update = "UPDATE evento SET nombre_evento = '". $nuevo_nombre."' WHERE idevento ='".$idevento."'  limit 1";		
	return  $this->db->query($query_update);
	
}
function updateEdicion($nuevo_edicion , $idevento ){

	$query_update = "UPDATE evento SET edicion = '". $nuevo_edicion."' WHERE idevento ='".$idevento."'  limit 1 ";		
	return  $this->db->query($query_update);
	
}
function updateDescripcion($nueva_descripcion , $idevento ) {
	
	$query_update = "UPDATE evento SET descripcion_evento='". $nueva_descripcion."' WHERE idevento ='".$idevento."'  limit 1";		
	return  $this->db->query($query_update);
...
function updateUbicacion($nueva_ubicacion , $idevento ) {
	...
	$query_update = "UPDATE evento SET ubicacion='". $nueva_ubicacion ."' WHERE idevento ='".$idevento."' limit 1";		
	return  $this->db->query($query_update);
