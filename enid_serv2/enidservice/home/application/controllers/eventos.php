...
class Eventos extends CI_Controller{
    ...
    /*Búsqueda de eventos*/    
    function busqueda(){     
        $data = $this->validate_user_session_event("Encuentra tus eventos");                                
...
function diaevento($id_evento){
        ...
        if ($this->checkifexist($id_evento ) == 1 ) {
            ...
            $data = $this->validate_user_session_event("El día del evento");            
...
/*Pre visualizar  ********************** pre visualizar */
    function visualizar($id_evento){
        ...
        if ($this->checkifexist($id_evento) == 1 ) {
                ...
                $data =  $this->validate_user_session_event($dataevent[0]["nombre_evento"] );                                                                        
    ...
    function config_eventos($id_evento){
        ...
        $data = $this->validate_user_sesssion("Servicios que incluirá el evento"); 
        $data["evento"] =  $id_evento;
        $this->dinamic_view_event('eventos/accesos_configs', $data);           
    }
