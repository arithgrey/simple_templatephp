...
if(!function_exists('invierte_date_time')){
	...
	function reporte_cuadro_global($data){		
		...
		if (count($data["fechas"]) > 0 ) {
			...
			$tb .="<table class='table table-bordered' style='background:#13BAFC; color:white;'>";					
			$tb .=  get_vector_fechas($data["fechas"]); 		
			$tb .="</table>";			
		}
		$tb .=  get_table_global($data["info"]);	
		return $tb;
	}	
	/**/
	function get_vector_fechas($data){	
		$h =  "<tr>"; 		
		$h .= get_td("<a style='color:white; font-weight: bolder; font-size: .9em;' > + fechas</a>" ,"");
		foreach ($data as $row){
			$h .=  get_td( "<a style='color:white; font-weight: bolder; font-size: .9em;' class='f_busqueda' id='".$row["fecha"] ."'>". $row["fecha"] ."</a>", "");			
		}
		$h .=  "</tr>"; 		
		return $h;
	}
	...
	function get_table_global($data){
		...
		foreach ($data as $row) {	
			...
			$tb .= get_td($nombre_evento ,"class='franja-vertical'");
			$tb .= get_td($edicion ,"class='dinamic_campo_tb'");			
