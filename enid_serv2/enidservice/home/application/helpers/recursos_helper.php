...
function lista_usuarios_cuenta($integrantes , $limit ){
    ...
    foreach ($integrantes as $row) {
        ...
        $list .="<tr class='text-center'>";      
            ...
            /*********************/
            $list .= get_td( create_img_user($row), 'class="prog-avatar"');    
            /*********************/
    ...
    if ($b > 9){
    	$list .='<tr class="text-center enid-header-table" >';
                    ...
                    $list .= get_td( "Usuario" , "" ); 
                    $list .= get_td( "Edad" , "" ); 
                    $list .= get_td( "Registro" , "class='dinamic_campo_tb' "  ); 
                    $list .= get_td( "Perfil" , "" ); 
                    $list .= get_td( "Tel" , "" );
                    $list .= get_td( "Tel. alterno"  , "class='dinamic_campo_tb' "  );
                    $list .= get_td( "# Empleado" , "" );
   ...
   $complete .=  "<div class='row'>
                    <div class='col-lg-12'>" . limits_tables("index.php/recursocontroller/usuarios" , $limit ) ."</div>
                    </div>";                 
   $complete .= "<em class='total_table'>Montrando: ". $now ."</em>
                <a class='pull-right mas-info'> + info</a>
                <a class='pull-right menos-info' style='display:none;'> - info</a>";                 
   $complete .= $list;
    return $complete;
}
/**/
function create_img_user($row){

    $user_img ="";            
    if($row["nombre_imagen"] != null){                                                                       
        $img = base_url($row["base_path_img"] . $row["nombre_imagen"]);                    
        $complate_img = create_icon_img($img, "img_user", $row["idusuario"], "a", 1 );
        $user_img .= $complate_img;                        
    }else{
        $user_img .='<i class="img_user fa fa-cloud-upload" id="'. $row["idusuario"] .'" ></i>';             
    }                   
    return $user_img;    
}
