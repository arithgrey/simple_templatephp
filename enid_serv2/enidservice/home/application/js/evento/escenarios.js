function  nuevo_escenario(){
	...
	$.post(url , $("#form-escenario").serialize() ).done(function(data){
		...
		reset_fields(fields); 
		$("#alert-escenario-ok").show();
		muestra_alert_segundos("#alert-escenario-ok"); 
...
function load_data_escenarios(){	
	...
	$.get(url , $("#form-escenario").serialize() ).done(function(data){
		...
		alert("Error al cargar escenario");
	});
}
/**/
...
function delete_escenario(e){
	$("#aceptar-delete").click(function(){
		...
		$.post(url , {"idescenario" :  idescenario } ).done(function(data){
