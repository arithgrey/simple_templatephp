...
function carga_logo_empresa($data_empresa , $in_session ){
  ...
  if (strlen($data_empresa["nombre_imagen"]) > 10 && $data_empresa["nombre_imagen"] != null ) {
    $path_img = $data_empresa["base_path_img"]. $data_empresa["nombre_imagen"];       
    $url_img =  base_url($path_img); 

    $img = "<img src='".$url_img."' title='Logo de la empresa' id='logo_empresa_img' $modal  >";
    
  }else{
    /*Creamos la simulación de la imagen */
    $img = '<div class="img-tmp-empresa" title="Logo de la empresa"   $modal  >
              <br>
              <center> 
