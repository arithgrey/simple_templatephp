<?php defined('BASEPATH') OR exit('No direct script access allowed');
 class contactmodel extends CI_Model {  
  ...
  function get_contacto_q($id_usuario ,  $q){ 
    ...
    $query_get ="SELECT c.* , i.idimagen, i.nombre_imagen , i.base_path , i.base_path_img   FROM contacto c  
                left outer  join  imagen_contacto ic on 
                c.idcontacto =  ic.id_contacto 
                left outer  join imagen  i  on  ic.id_imagen =  i.idimagen 
                where  idusuario ='".$id_usuario."'  and c.nombre like '%".$q."%' 
  ...
  function get_contactos_user($idusuario , $param  , $limit=10 ){
     ...
     if($limit != "all" ) {
        $filtro_text =" limit " . $limit;  
      }else{
        $filtro_text ="";
    }
    /**/
    $filtro ="";
    if ( strlen($param["contacto-name"]) > 0  && strlen($param["contacto-tel-filtro"] ) < 3  ) {
      $filtro= " and nombre  like '". $param["contacto-name"] ."%' " ;  

    }else if(strlen($param["contacto-tel-filtro"]) > 0  && strlen($param["contacto-name"]) <  3  ){

      $filtro= " and  tel like '". $param["contacto-tel-filtro"] ."%' " ;    

    }else if(strlen($param["contacto-tel-filtro"] ) > 0  && strlen($param["contacto-name"]) > 0  ){
      $filtro= " and  nombre  like '". $param["contacto-name"] ."%'  OR   tel like '". $param["contacto-tel-filtro"] ."%'  " ;  
    
    }else if(strlen($param["contacto-name"]) == 0  and  strlen($param["contacto-tel-filtro"]) == 0 ){
     
      $filtro = " and  tipo like '". $param["filtro-tipo-contacto"] ."%'" ;            
    }else{    
    }    
    ...
    $query_get ="SELECT c.*,
                    ...
                    where idusuario ='".$idusuario."'  ".$filtro." " . $filtro_text;                    
  ...
  /**/
  /*
  function cargamos_base_img_contactos(){    
    
    $query_procedure ="CALL enidserv_eniddbbbb3.create_tmp_img(6);";
    $this->db->query($query_procedure);
  }
  */
  /*Lista de contactos*/
  ...
  /**/
  function update( $nombre , $organizacion , $telefono, $movil, $correo , $direccion, $tipo , $idusuario, $nota , $pagina_web ,  $id_contacto , $pagina_fb , $pagina_tw , $correo_alterno , $extension ){
