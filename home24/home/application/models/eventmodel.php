...
from    
v_eventos_escena ee 
left outer join v_escenarios_artistas_e ea  on  ee.idevento  =   ea.idevento			
left outer join v_evento_punto_v ep on ee.idevento = ep.idevento	
left outer join  v_evento_sound es  on ee.idevento = es.idevento 
left outer join v_event_serv eserv  on  ee.idevento = eserv.idevento
left outer join v_accesos va  on ee.idevento = va.idevento";
...
function get_img_evento($id_evento){
	...
	$query_get ="select i.* from imagen i inner join imagen_evento ie on  i.idimagen =  ie.id_imagen inner join evento e on ie.id_evento= e.idevento where e.idevento = '". $id_evento."'  limit 1";
	$result = $this->db->query($query_get);
	return $result->result_array();
...
function get_last_events($id_empresa , $num ){
	...
	, CONCAT(i.base_path_img , i.nombre_imagen)portada      from repo_eventos_escenarios_$_num e
