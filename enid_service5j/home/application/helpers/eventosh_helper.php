...
function get_slider_img_evento($data){
	...
	/**/
	function create_text_slogan( $slogan , $in_session ){
		$msj = ''; 
		$seccion = ''; 
		if ($in_session ==1  ) {						
			$msj =  "No has registrado un slogan para el evento. "; 
			if (strlen($slogan)> 0) { $seccion =  seccion_slogan($slogan);}else{$seccion =  seccion_slogan($msj);}		
		}else{			
			if (strlen($slogan) > 0 ) {$seccion =  seccion_slogan($slogan);	}
		}
		return $seccion; 
	}
	/**/
	function seccion_slogan($msj){

		$seccion =  '<h4 class="pull-right">
						<small class="slogan">                   
						'. $msj .'
						</small>
					</h4>';
	...
	/**/
	function create_text_descripcion($descripcion , $generos_musicales_tags , $in_session ){
		
		$text =  c_seccion_descripcion($descripcion , $in_session ); 
		$seccion = " 		
		<div class='col-lg-12 col-md-12 col-sm-12  descripcion_seccion' >
		    <div class='panel'>
		        <div class='panel-body'>
		            <div>
		  				". $text  ."
	...
	/**/
	function c_seccion_descripcion($descripcion , $in_session ){		
		$text =  trim($descripcion); 
		$new_text =  $descripcion; 
		if ($in_session == 1 ) {
			if (strlen($descripcion) < 5 ){
				$new_text = 'No has cargado la descripción del lo que vivirá el publico al asistir al evento.'; 		
			}
		}else{
			if (strlen($descripcion) < 5 ){
	...
	/**/
	function contruye_ubicacion($ubicacion , $in_session ){
		/**/
		$span =  ""; 
