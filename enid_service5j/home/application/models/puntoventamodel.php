...
/*Actualiza todo los puntos de venta asociados al evento */
	...
	/**/
	function insert($razon_social,  $status, $descripcion, $zona,   $id_usuario, $id_empresa ){

		$query_insert = "call insert_punto_venta_empresa_usuario('".$razon_social."' , '". $status ."'   ,  '". $id_empresa ."' ,  '".$descripcion ."' , '". $zona ."' , '".$id_usuario."'   )"; 		
	...
	function get_puntos_venta_empresa_usuario($id_empresa, $param , $limit){
		...
		$filtro = $this->filtro_contacto($param);
		/*Cargamos  la base temporal para img */
		$_num = $this->cargamos_base_img_puntos_venta(4 , 0);
		/*Terminamos de cargar  la base temporal para img */
		$query_get ="select pv.*, 
	...
	/**/
	function filtro_contacto($param){

		$filtro =""; 
		//puntos_venta_b=&estado_punto_venta_b=Disponible+para+todos+los+colaboradores+de+la+empresa&zona_b=-
		
		$limit_text ="";		
		if (strlen($param["puntos_venta_b"]) > 0  ) {
			$filtro .=" and  pv.razon_social like '". $param["puntos_venta_b"] ."%'   ";	
		}if (strlen($param["estado_punto_venta_b"]) > 0  ) {
			$filtro .=" and  pv.status  like '". $param["estado_punto_venta_b"] ."%'   ";	
	...
	function update($razon_social,$status ,  $descripcion, $zona,  $id_usuario,  $id_empresa , $id_punto_venta){
