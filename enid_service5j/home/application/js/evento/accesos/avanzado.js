	...
	if (flag ==  1 ){
		...
		if (flag_numerico == 1 ){
			...
			$.ajax({
				...
				show_response_ok_enid(".place_registro_acceso" ,  "Escenario registrando con éxito.! "); 									
...
function actualiza_data_accesos(){
	...
	$.ajax({
	   ...
	   beforeSend : function(){
		...
		show_response_ok_enid(".place_editar_acceso" , "Datos del acceso actualizados con éxito");
		load_data_accesos();
