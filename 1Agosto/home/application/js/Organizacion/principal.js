		...
		if (val.length > 0 ){				
				...
				$.ajax({
				   ...
				   data : data_send  }).done(function(data){
					...
					/**/
					$("#mensaje-comunidad-response").show();
					muestra_alert_segundos("#mensaje-comunidad-response");


				}).fail(function(){
				   	alert("falla al intentar actualizar");
				   	$("#alert-fail-mensaje-comunidad").show();
				});


		}else{
				llenaelementoHTML(".text-edit-mensaje-comunidad" , "Nombre de tu evento"  ); 					
		}
