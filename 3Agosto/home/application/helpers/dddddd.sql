...
begin
  IF flag = 0 THEN 
    ...
    SET @tmp =  CONCAT('create table tmp_img_' ,  num , ' as select idimagen, nombre_imagen, base_path, base_path_img from
      imagen where type =' , a ); 
    PREPARE stmtd from @tmp; 
      EXECUTE stmtd;
...
begin
    IF flag = 0 THEN 
        ...
        SET @tmp =  CONCAT('create table tmp_contenido_' ,  num , ' 
                        as 
                        select  * from contenido where type="7" and
                        DATE(fecha_registro) 
                        BETWEEN  DATE_ADD(CURRENT_DATE() , 
                        INTERVAL -1  MONTH) AND  CURRENT_DATE()'); 
        PREPARE stmtd from @tmp; 
...
INSERT INTO `enidserv_eniddbbbb3`.`perfil_permiso` (`idperfil`, `idpermiso`) VALUES (3, 27);
