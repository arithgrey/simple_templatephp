...
function get_servicio_evento($param){
	$query_get =   "SELECT * FROM evento_servicio WHERE idevento = '". $param["evento"] ."' AND  idservicio = '".$param["servicio"] ."'   limit 1";
...
function get_servicios_evento_config($id_evento , $id_empresa){
	$query_get = "select s.idservicio id_servicio  ,  s.servicio  ,  ev.* , ev.idevento id_evento ,  ev.idservicio  eventoservicio ,  ev.fecha_registro  fecha_servicio_registro  
	  	from servicio as s
		left outer join evento_servicio  as ev 
		on s.idservicio = ev.idservicio and ev.idevento = '". $id_evento ."' ";	
	$result = $this->db->query($query_get);
	return  $result ->result_array();
}
/**/
