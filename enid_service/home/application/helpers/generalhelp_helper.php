...
if(!function_exists('invierte_date_time')){
  ...
  /**/
  function create_icon_img($url , $class, $id , $title , $flag , $letra ='A' ){  
    //$color = substr(md5(time()), 0, 6);    
    //$color_random =  'background : #'.$color . ';';
    $color_random = 'background : rgb(174, 13, 80);';
    if ($flag == 1  ){            
      return  "<img   class='img-icon-enid  ". $class ." ' id='".$id  ."' src='" . $url . "' title='". $title ."'>";  
    }else{
      /*Generamos color al de fondo */           
      return  "<div style = '". $color_random."' class='img-icon-enid text-center ". $class ." ' id='".$id  ."'  title='". $title ."' >". $letra ."</div>";
    }
          ...
          foreach ($tupla as $key => $value){
            ...
            $valor   =  $row[$param]; 
              ...
              if($b == 0 ){              
                ...
                $url_img  = $row["base_path_img"]. $row["nombre_imagen"];  
                $filtro .= create_icon_img( base_url($url_img) ,  "" , ""  , ""  , 1  );              
                $b ++; 
