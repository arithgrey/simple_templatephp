...
function load_puntos_venta_complete($data){
		...
		foreach ($data as $row) {
			...
			$list.="<tr>";
					...
					if($row["nombre_imagen"] != null ){
						...
						$img =  '<img class="img_punto_venta fa fa-cloud-upload"  id="'. $idpunto_venta .'"    style="width:30px;" src="data:image/jpeg;base64, '. base64_encode($row["img"])  .'  " />';
						$list .=  get_td( $img , "data-toggle='modal' data-target='#punto-venta-imagen-modal' ");
					}else{
	...
	/**/
	function lista_zonas_punto_venta()
	{
