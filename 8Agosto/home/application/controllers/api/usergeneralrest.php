...
class Usergeneralrest extends REST_Controller{
	...
	function validaterecordusergeneral_post(){
        ...
        if ($existecompañia == 1 ) {
            ...
            $isuser = $this->isuserexist($id_user , $mail);


                if (!$isuser){
                    $idempresaregistrada = $this->recordempresa($empresaname);                                     
                    $responsedb = $this->recordusergeneral($id_user ,  $mail  , $pw , $idempresaregistrada );
                        if ($responsedb == 1 ){
    ...
    function isuserexist($id_user ,  $mail){

        $responsedb = $this->usuariogeneralmodel->validaexistuser($id_user , $mail );                
        if ($responsedb >0) 
            return true;  
        else
            return false;                
