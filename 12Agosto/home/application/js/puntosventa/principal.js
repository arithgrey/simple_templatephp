$(document).on("ready", function(){
	...
	$(".contacto_direccion").keyup(function(){
		carga_maps(".contacto_direccion", "direcciones-disponibles" , ".place_lista_direcciones");
	})
	...
	url =  now + "index.php/api/puntosventa/punto_venta/format/json/"; 
		...
		$.ajax({
		   ...
		   beforeSend : function(){
			...
			valorHTML(".locacion" , data[0].descripcion);   	
			$("#form-nota-pv").submit(actualiza_nota_punto_venta);

			$(".locacion").keyup(function(){
				carga_maps(".locacion", "locaciones" , ".list-locaciones");
			});
